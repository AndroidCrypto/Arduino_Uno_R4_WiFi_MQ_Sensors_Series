...
//Declare Sensor
MQUnifiedsensor MQ3(Board, Voltage_Resolution, ADC_Bit_Resolution, pin, MQType);
...
*/
  ...
  /*****************************  MQ CAlibration ********************************************/
  Serial.println("** Values from  MQ-3  ****");
  Serial.println(F("|   LPG    |    CH4    |    CO    |  Alcohol |  Benzene |  Hexane  |"));
...
void loop() {
  ...
  lpg = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup

  //MQ3.setA(2*10^31);
  //MQ3.setB(19.01);
  //float Alcohol = MQ135.readSensor(); // SSensor will read PPM concentration using the model, a and b values set previously or from the setup
  //ch4 = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  // Note: the values for 'setA' and 'setB' are giving an 'ovf' error

  MQ3.setA(521853);
  MQ3.setB(-3.821);
  //float CO2 = MQ135.readSensor(); // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  co = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup

  MQ3.setA(0.3934);
  MQ3.setB(-1.504);
  //float Toluen = MQ135.readSensor(); // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  alcohol = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup

  MQ3.setA(4.8387);
  MQ3.setB(-2.683);
  //float NH4 = MQ135.readSensor(); // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  benzene = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup

  MQ3.setA(7585.3);
  MQ3.setB(-2.849);
  //float Aceton = MQ135.readSensor(); // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  hexane = MQ3.readSensor();  // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  Serial.print("|   ");
  Serial.print(lpg);
  Serial.print("   |   ");
  Serial.print(ch4);
...
void updateThingSpeakChannel() {
  ...
  if (client.connect(server, 80)) {
    String getData = "api_key=" + writeAPIKey + "&field1=" + String(lpg) + "&field2=" + String(ch4) + "&field3=" + String(co) + "&field4=" + String(alcohol) + "&field5=" + String(benzene) + "&field6=" + String(hexane);
...
/*
|   LPG    |    CH4    |    CO    |  Alcohol |  Benzene |  Hexane  |
